...
namespace aclogview
{
    ...
    /// </summary>
    public class CryptoSystem2: ICryptoSystem
    {
        ...
        /// </summary>
        private IISAACProvider Wheel = null;
        /// <summary>
        /// The number of times the stream cipher has advanced.
        /// </summary>
        private int InternalOffset => Wheel.GetValuesTakenCount();
        /// <summary>
        ...
        /// </summary>
        private SortedDictionary<int, int> Cache = new SortedDictionary<int, int>();
        ...
        public int ConsumeKey(uint xor)
        {
            ...
            if (Search(cbSearch, true))
            {
                int keyOffsetForLogging = verifiedKey.Item1;
                return keyOffsetForLogging;
            }
            return -1;
        }
        public CryptoSystem2() { }
        public CryptoSystem2(int seed, ISAACProvider provider)
        {
            Init(BitConverter.GetBytes(seed), provider);
        }

        public CryptoSystem2(uint seed, ISAACProvider provider)
        {
            int signed_seed = unchecked((int)seed);
            Init(BitConverter.GetBytes(signed_seed), provider);
        }

        private void Init(byte[] seed, ISAACProvider provider)
        {
            if (seed == null)
            {
        ...
        private CacheResult this[int offset]
        {
            get
            {
                ...
                {
                    added = (int)Wheel.Next();
                    Cache.Add(InternalOffset - 1, added.Value);
                }
                if (added != null)
                {
                    return new CacheResult() { Key = new Tuple<int, int>(InternalOffset - 1, added.Value) };
                }
                ...
                return new CacheResult() { Key = new Tuple<int, int>(offset, Cache[offset]) };
            }
        }
        ...
        public bool Search(Func<Tuple<int, int>, bool> callback, bool rangeAdvance)
        {
            Tuple<int, int> range = SearchRange;
